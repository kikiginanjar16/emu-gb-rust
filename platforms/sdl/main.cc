...
static std::string get_save_filename() { return cliOptions.filename + ".sav"; }
...
static void save_state() {
    ...
    std::copy(cartridge_ram.begin(), cartridge_ram.end(),
        ...
        switch (event.type) {
            case SDL_KEYDOWN:
                ...
                if (event.key.keysym.sym == SDLK_F6) {
                    ...
                    if (!save_data.empty()) {
                        gameboy->load_save_data(save_data);
                    }
                    break;
                }
...
static bool is_closed() { return should_exit; }
...
int main(int argc, char* argv[]) {
    ...
    window = SDL_CreateWindow("gbemu", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width,
    ...
    gb_screen_texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888,
    ...
    log_info("");
    log_info("Controls:");
    log_info("  Arrow keys : D-pad");
    log_info("  X          : A");
    log_info("  Z          : B");
    log_info("  Backspace  : Select");
    log_info("  Enter      : Start");
    log_info("  F5         : Save Game (Battery)");
    log_info("  F6         : Load Game (Battery)");
    log_info("");
