...
Cartridge::Cartridge(std::vector<u8> rom_data, const std::vector<u8>& ram_data,
    ...
    if (!ram_data.empty()) {
        if (ram_data.size() != ram_size_for_cartridge) {
            fatal_error("Invalid or corrupted RAM file. Read %d bytes, expected %d",
                        ram_data.size(), ram_size_for_cartridge);
        }
        ram = ram_data;
    } else {
...
void Cartridge::set_cartridge_ram(std::vector<u8> new_ram) { ram = std::move(new_ram); }
